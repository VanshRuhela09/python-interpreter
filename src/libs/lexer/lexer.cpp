...
Lexer::Lexer(const std::string& source) 
    ...
    /*
     *  keyword ::= "aur" | "del" | "from" | "not" | "graphic"
     *            | "as" | "elif" | "global" | "ya" | "with"
     *            | "assert" | "else" | "if" | "pass" | "yield"
     *            | "break" | "except" | "import" | "show"
     *            | "class" | "in" | "raise" | "continue" 
     *            | "finally" | "is" | "return" | "fn" 
     *            | "for" | "lambda" | "try"
    */
    keywords = {
...
std::vector<Token> Lexer::scanTokens() {
    ...
    isBlock = false;  //isBlock tracks whether the lexer is inside a block (e.g., after if, while, or {)
    indentLevels.push(0); // Initial indentation level
    tokens.clear();
    
    handleIndentation(); // at the beginning
    
    while (!isAtEnd()) {  //Continues scanning until the end of the source string is reached (checked via isAtEnd()).
        start = current;
        scanToken();
    }
